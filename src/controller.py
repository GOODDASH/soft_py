...
class Controller:
    ...
    def on_connect_nc_status(self, flag):
        if flag[0]:
            ...
            self.view.statusBar().showMessage("连接机床成功", 3000)
        else:
    ...
    def on_connect_tem_card_status(self, flag):
        if flag[0]:
            ...
            self.view.statusBar().showMessage("连接采集卡成功", 3000)
        else:
    ...
    def on_open_port_status(self, flag):
        if flag:
            ...
            self.view.statusBar().showMessage("打开串口成功", 3000)
        else:
    ...
    def on_start_sample(self, para):
        self.view.statusBar().showMessage("开始采集数据", 3000)
    ...
    def on_stop_sample(self):
        self.view.statusBar().showMessage("采集停止", 3000)
        for btn in self.view.side_menu.btns:
            btn.setEnabled(True)
    ...
    def on_import_data(self, para):
        ...
        self.view.statusBar().showMessage(f"从{para[0]}导入数据", 3000)
        self.view.setCursor(Qt.CursorShape.ArrowCursor)

    def on_plot_files(self):
        self.view.tsp_page.plot_files(self.state.data.data_arrays)
        self.view.statusBar().showMessage("已图示所有导入的数据", 3000)
    ...
    def on_tra_tsp(self, para):
        ...
        self.view.statusBar().showMessage(f"传统测点筛选完成:{self.state.tsp_res}", 2000)
    ...
    def on_saved_data(self, para):
        ...
        if not success:
            ...
            QMessageBox.information(self.view, "保存成功", message)
    ...
    def on_mlr_fit(self):
        ...
        if not success:
            ...
            self.view.statusBar().showMessage("已显示筛选温度点曲线和多元线性回归拟合结果", 2000)
    ...
    def on_import_model(self, para):
        ...
        self.view.statusBar().showMessage("模型导入完成", 2000)
